...
def MainDataImport():
    ...
    covidCanadaTotalCases_df = pandas.read_csv("~/Desktop/COVIDualize/CanadaCovid.csv")
    covidCanadaTotalDeaths_df = pandas.read_csv("~/Desktop/COVIDualize/CanadaCovidDeath.csv")
    covidCanadaTotalRecovered_df = pandas.read_csv("~/Desktop/COVIDualize/CanadaCovidRecovered.csv")
    # Remove the columns we don't care about
    covidCanadaTotalCases_df = covidCanadaTotalCases_df.drop(['Country/Region', 'Lat', 'Long'], axis=1)
    ...
    covidAB_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Alberta']).drop(['Province'], axis=1))).transpose()
    covidAB_df.columns = columninds
    covidAB_df.index.name = 'Date'
    covidAB_df.reset_index(inplace=True)
    covidBC_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'British Columbia']).drop(['Province'], axis=1))).transpose()
    covidBC_df.columns = columninds
    covidBC_df.index.name = 'Date'
    covidBC_df.reset_index(inplace=True)
    covidMN_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Manitoba']).drop(['Province'], axis=1))).transpose()
    covidMN_df.columns = columninds
    covidMN_df.index.name = 'Date'
    covidMN_df.reset_index(inplace=True)
    covidNB_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'New Brunswick']).drop(['Province'], axis=1))).transpose()
    covidNB_df.columns = columninds
    covidNB_df.index.name = 'Date'
    covidNB_df.reset_index(inplace=True)
    covidNL_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Newfoundland and Labrador']).drop(['Province'], axis=1))).transpose()
    covidNL_df.columns = columninds
    covidNL_df.index.name = 'Date'
    covidNL_df.reset_index(inplace=True)
    covidNS_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Nova Scotia']).drop(['Province'], axis=1))).transpose()
    covidNS_df.columns = columninds
    covidNS_df.index.name = 'Date'
    covidNS_df.reset_index(inplace=True)
    covidON_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Ontario']).drop(['Province'], axis=1))).transpose()
    covidON_df.columns = columninds
    covidON_df.index.name = 'Date'
    covidON_df.reset_index(inplace=True)
    covidPE_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Prince Edward Island']).drop(['Province'], axis=1))).transpose()
    covidPE_df.columns = columninds
    covidPE_df.index.name = 'Date'
    covidPE_df.reset_index(inplace=True)
    covidQC_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Quebec']).drop(['Province'], axis=1))).transpose()
    covidQC_df.columns = columninds
    covidQC_df.index.name = 'Date'
    covidQC_df.reset_index(inplace=True)
    covidSA_df = (((covidCanadaTotalCases_df.loc[covidCanadaTotalCases_df['Province'] == 'Saskatchewan']).drop(['Province'], axis=1))).transpose()
    ...
    covidCanadaTotalCases_df = covidCanadaTotalCases_df.drop(['Province'], axis=1)
    ...
    covidCanadaTotalDeaths_df = covidCanadaTotalDeaths_df.drop(['Province', 'Country/Region', 'Lat', 'Long'], axis=1)
    covidCanadaTotalDeaths_df = covidCanadaTotalDeaths_df.sum(axis=0, skipna=True)
    ...
    return covidCanadaTotal_df, covidAB_df, covidBC_df, covidMN_df, covidNB_df, covidNL_df, covidNS_df, covidON_df, covidPE_df, covidQC_df, covidSA_df

def WorstCaseDataImport():
    covidWorseTotal_df = pandas.read_csv("~/Desktop/COVIDualize/WorstCaseData.csv")
    covidWorseTotalDeaths_df = pandas.read_csv("~/Desktop/COVIDualize/WorstCaseDeaths.csv")
    covidWorseTotalRecovered_df = pandas.read_csv("~/Desktop/COVIDualize/WorstCaseRecovery.csv")
    ...
    covidWorseTotalDeaths_df = covidWorseTotalDeaths_df.sum(axis=0, skipna=True)
...
def BestCaseDataImport():
    covidBestTotal_df = pandas.read_csv("~/Desktop/COVIDualize/BestCaseData.csv")
    covidBestTotalDeaths_df = pandas.read_csv("~/Desktop/COVIDualize/BestCaseDeaths.csv")
    covidBestTotalRecovered_df = pandas.read_csv("~/Desktop/COVIDualize/BestCaseRecovery.csv")
